...
class PurchaseConsts {
  static const String isNotAvailable =
      'The store cannot be reached or accessed.';
  static const String noSubscription = 'No subscriptions';
...
class SubsPurchase {
  ...
  Future<void> initialize(List<String> productIds, List<String> onetimeSubIds,
    ...
    try {
      ...
      if (!_isAvailable) {
        splog(PurchaseConsts.isNotAvailable);
        return;
      }
  ...
  Future<void> purchaseProduct(
    ...
    if (!_isAvailable) {
      callback?.call(PurchaseResult(
        ...
        message: PurchaseConsts.isNotAvailable,
      ));
      return;
    }
    ...
    if (product == null) {
      callback?.call(PurchaseResult(
        status: IAPPurchaseStatus.purchaseFailed,
        message: PurchaseConsts.productNotFound,
      ));
      return;
    }
      ...
      if (latestPurchasedProducts.isEmpty) {
        Future.delayed(const Duration(seconds: 2), () {
          _notifyCallback(IAPPurchaseStatus.restoreFailed,
              message: PurchaseConsts.noSubscription);
        });
        return;
      }
  ...
  void _handlePurchaseUpdates(List<PurchaseDetails> purchaseDetailsList) {
    for (var purchaseDetails in purchaseDetailsList) {
      if (purchaseDetails.status == PurchaseStatus.pending) {
        ...
        final message = _handlePurchaseError(
            message: purchaseDetails.error?.message,
            detail: purchaseDetails.error?.toString());
        _notifyCallback(IAPPurchaseStatus.systemError,
            message: message, data: purchaseDetails.error?.toString());
  ...
  /// [message] 错误信息
  /// [detail] 错误详情
  String _handlePurchaseError({String? message, String? detail}) {
    if (detail == null) {
      return message ?? '';
    }
    final re = RegExp(
        r'(?:NSLocalizedDescription|NSDebugDescription|NSLocalizedFailureReason)\s*[:：]\s*(.*?)\s*(?=(?:\s*,?\s*NS|AMS|}|$))');

    var result = '';
    for (final match in re.allMatches(detail)) {
...
extension SubsPurchaseExtension on SubsPurchase {
  /// 获取订阅标题
  ///
  /// [duration] 订阅周期
  ///
  ...
  ///
  /// [isShort] 是否为短标题
  String getSubcriptionTitle(SubscriptionDuration duration, String languageCode,
      {bool isShort = false}) {
    return SubsLocale.subscriptionTitle(duration.name, languageCode,
        isShort: isShort);
  }

  /// 获取本地化副标题
  ///
  /// 免费试用期+价格，比如：Free trial 1 week, then $1.99/week
  ///
  /// [product] 产品信息
  String getSubcriptionSubTitle(ProductDetails product, String languageCode) {
    if (Platform.isIOS) {
      ...
      if (_onetimeSubsIds.contains(product.id)) {
        //终身会员
        return SubsLocale.defaultSubtitle(appStoreProduct.skProduct,
